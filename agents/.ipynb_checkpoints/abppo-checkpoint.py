...
class BehaviorPPO:
    ...
    def update(self, s, a, adv, current_clip_ratio):
        with torch.no_grad():
...
class AdaptiveBehaviorPPO:
    ...
    def joint_train(self, buffer, batch_size, iql_net, current_clip_ratio):
        s, _, _, _, _ = buffer.sample(batch_size)
        losses = []

        for i, bppo in enumerate(self.ensemble):
            # 1. 用 old_policy 采样动作 (离线 PPO 的核心机制)
            with torch.no_grad():
            ...
            # 3. 更新策略
            loss = bppo.update(s, a, adv, current_clip_ratio)
            losses.append(loss)
