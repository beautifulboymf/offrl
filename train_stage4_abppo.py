...
class Stage4Args:
    def __init__(self, full_env_name, device):
        ...
        # OPE evaluating hyperparameters, Uni-O4: eval_step = 100 rollout_length = 1000
        self.eval_step = 100
        self.rollout_length = 1000
        self.rollout_batch_size = 512
        
        # Transition model and core training parameters
        self.n_ensemble = 7
        self.n_elites = 5
...
def main():
    ...
    # 1. Load Dynamics Model from Stage 3
    dynamics = train_dynamics(args, env, eval_buffer)

    # 2. Load IQL from Stage 1
    ...
    # 3. Load BC Ensemble from Stage 2
    ...
    # Sync BC weights to ABPO policy and old_policy
    ...
    for step in pbar:
        # Linear decay for learning rate and clip ratio
        ...
        # Trigger AM-Q evaluation every 100 steps
        if (step + 1) % args.eval_step == 0:
            current_qs = np.zeros(4)
            for i in range(4):
                # Perform dynamics rollout
            ...
            # Policy replacement logic: update behavioral baseline only when virtual environment score improves
    ...
    # Save the final optimized best policy
